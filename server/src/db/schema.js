...
const conversationMemberRoleEnum = pgEnum("conversation_member_role", [
  "owner",
  "admin",
  "member",
]);
...
const friendRequests = pgTable(
  ...
  (table) => [
    ...
    index("friend_requests_receiver_created_idx").on(
      table.receiverId,
      table.createdAt.desc(),
    ),
    index("friend_requests_sender_created_idx").on(
      table.senderId,
      table.createdAt.desc(),
    ),
  ],
...
const conversations = pgTable(
  ...
  (table) => [
    ...
    check(
      "conversations_direct_distinct_users",
      sql`${table.directUserOneId} IS NULL OR ${table.directUserOneId} <> ${table.directUserTwoId}`,
    ),
    check(
      "conversations_direct_canonical_pair",
      sql`${table.directUserOneId} IS NULL OR ${table.directUserOneId} < ${table.directUserTwoId}`,
    ),
...
const conversationMembers = pgTable(
  ...
  (table) => [
    ...
    index("conversation_members_user_active_idx")
      .on(table.userId, table.joinedAt.desc())
      .where(sql`${table.leftAt} IS NULL`),
    index("conversation_members_conversation_active_idx")
...
const conversationMembersRelations = relations(
  ...
  ({ one }) => ({
    ...
    lastReadMessage: one(messages, {
      ...
      references: [messages.id],
      relationName: "member_last_read_message",
    }),
  }),
);
