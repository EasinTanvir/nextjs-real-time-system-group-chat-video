...
const requireUser = async (id) => {
  ...
  if (!user) throw new AppError(401, "Authenticated user was not found.");
  return user;
};
...
const updateSettings = async (id, values) => {
  ...
  const [settings] = await db
    .insert(userSettings)
    .values({ userId: id, ...values, updatedAt: new Date() })
    .onConflictDoUpdate({
...
const listUsers = async (actorId, { search, limit, cursor }) => {
  const filters = [
    gt(users.id, cursor || "00000000-0000-0000-0000-000000000000"),
  ];
  if (search)
    filters.push(
      or(
  ...
  const result = await Promise.all(
    rows
      ...
      .map(async (user) => {
        const [one, two] =
          actorId < user.id ? [actorId, user.id] : [user.id, actorId];
        const friendship = (
          await db
            ...
            .where(
              and(
                eq(friendships.userOneId, one),
                eq(friendships.userTwoId, two),
              ),
            )
            .limit(1)
        )[0];
          ...
          await db
            ...
            .where(
              and(
                ...
                eq(friendRequests.receiverId, actorId),
                eq(friendRequests.status, "pending"),
              ),
            )
            .limit(1)
        ...
        return {
          ...publicUser(user),
          relationship: incoming ? "incoming_pending" : "none",
