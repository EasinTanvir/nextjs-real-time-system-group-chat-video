...
const registerMessageHandlers = (io, socket) => {
  registerEvent(socket, EVENTS.MESSAGE_SEND, async (payload) => {
    ...
    const content = string(payload?.content, "content", {
      required: true,
      max: 5000,
    });
    const result = await create(socket.data.userId, conversationId, content);
    io.to(conversationRoom(conversationId)).emit(EVENTS.MESSAGE_NEW, {
    ...
    result.notifications.forEach((notification) =>
      io
        ...
        .emit(EVENTS.NOTIFICATION_NEW, { notification }),
    );
  ...
  registerEvent(socket, EVENTS.MESSAGE_READ, async (payload) => {
    ...
    const membership = await markRead(
      socket.data.userId,
      conversationId,
      messageId,
    );
    socket
      .to(conversationRoom(conversationId))
      .emit(EVENTS.MESSAGE_READ_UPDATED, {
        conversationId,
        messageId,
        userId: socket.data.userId,
