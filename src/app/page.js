...
function BrandMark({ small = false }) {
  return (
    <span
      className={`grid place-items-center ${small ? "rounded-md h-5 w-5" : "rounded-lg h-8 w-8"} bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-[0_6px_14px_rgba(37,99,235,.25)]`}
    >
      <MessageCircle
    ...
    <aside className="hidden w-[188px] shrink-0 border-r border-slate-100 bg-white px-3 py-4 lg:block">
      ...
      <nav aria-label="Chat preview navigation" className="mt-4 space-y-0.5">
        {navItems.map(([Icon, label, active]) => (
          <div
            key={label}
            className={`flex h-7 items-center gap-2 rounded-md px-2 text-[10px] font-medium ${active ? "bg-blue-50 text-blue-600" : "text-slate-700"}`}
          >
      ...
      <div className="mt-5 flex items-center gap-2 border-t border-slate-100 pt-3">
        <Avatar initials="JD" color="from-slate-300 to-slate-100" size="md" />
    ...
    <section className="w-[220px] shrink-0 border-r border-slate-100 bg-white py-4 sm:w-[250px]">
      ...
      <div className="mt-4">
        {conversations.map((chat, index) => (
          <div
            key={chat.name}
            className={`flex gap-2 px-4 py-2.5 ${index === 0 ? "bg-blue-50/70" : ""}`}
          >
          ...
          <div className="px-5 py-5">
            ...
            <div className="ml-auto mt-4 w-fit">
              ...
              <p className="mt-1 flex items-center justify-end gap-1 text-[8px] text-slate-400">
                10:31 AM <CheckCheck className="h-3 w-3 text-blue-500" />
              </p>
            </div>
            <div className="mt-4 flex items-start gap-2">
              <Avatar initials="EJ" color="from-rose-300 to-orange-200" />
            ...
            <div className="ml-auto mt-4 w-fit">
              ...
              <p className="mt-1 flex items-center justify-end gap-1 text-[8px] text-slate-400">
                10:33 AM <CheckCheck className="h-3 w-3 text-blue-500" />
              </p>
            </div>
          </div>
...
export default function HomePage() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#fbfcff] text-slate-900">
      <div className="absolute inset-x-0 top-0 -z-0 h-[620px] bg-[radial-gradient(circle_at_80%_40%,rgba(219,234,254,.8),transparent_31%),radial-gradient(circle_at_47%_18%,rgba(238,242,255,.85),transparent_29%)]" />
      <div className="relative z-10 mx-auto max-w-[1390px] px-5 sm:px-8 lg:px-10">
        <header className="flex h-[78px] items-center border-b border-slate-200/80">
          <a
            href="#top"
          ...
          <nav
            ...
            className="mx-auto hidden items-center gap-12 text-[13px] font-medium lg:flex"
          >
            <a
              href="#features"
              className="transition hover:text-blue-600 focus-visible:text-blue-600"
            >
              Features
            </a>
            <a
          ...
          <div className="ml-auto hidden items-center gap-7 sm:flex">
            ...
            <a
              href="/register"
              className="rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 px-5 py-3 text-[13px] font-bold text-white shadow-[0_10px_20px_rgba(37,99,235,.22)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_24px_rgba(37,99,235,.3)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600"
            >
              Get Started
            </a>
          </div>
        ...
        <section
          id="top"
          className="grid items-center gap-12 pb-16 pt-12 lg:grid-cols-[.88fr_1.2fr] lg:gap-16 lg:pb-20 lg:pt-20"
        >
          <div className="max-w-[540px]">
            <div className="inline-flex items-center gap-2 rounded-full bg-blue-100/80 px-3 py-1.5 text-[11px] font-semibold text-blue-700">
              <span>✣</span> Modern chat for teams and friends
            </div>
        ...
        <div className="mx-auto max-w-[1080px] px-5 sm:px-8">
          ...
          >
            {[
              ...
              <div
                key={label}
                className={`flex items-center justify-center gap-4 py-2 ${index ? "sm:border-l sm:border-slate-300/70" : ""}`}
              >
